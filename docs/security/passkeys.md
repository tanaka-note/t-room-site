...
T-Cloudの通常本人利用は`folder-member`を使用する。パスキー本人確認とcredential固有のRSA鍵委譲で割当root folder keyを端末内復号できた時点で、そのrootは従来のfolder PWを再入力せず利用できる。配下に別PWを設定した子フォルダは新しい境界として従来どおり解除を要求する。PWによる`subadmin`は管理・互換・復旧経路として全トップフォルダを表示し、各保護フォルダPWで解除する従来動作を維持する。同一IdentityにCloud linkが1件なら自動選択し、複数なら選んだ1 linkだけのroot scopeでsessionを発行して権限を合成しない。
