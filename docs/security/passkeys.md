...
T-Cloudの通常本人利用は`folder-member`を使用する。パスキー本人確認とcredential固有のRSA鍵委譲で割当root folder keyを端末内復号できた時点で、そのrootは従来のfolder PWを再入力せず利用できる。配下に別PWを設定した子フォルダは新しい境界として従来どおり解除を要求する。第一管理者Identityは同じcredentialから`admin`または自身へlinkされた通常の`folder-member`を選択する。`admin`は管理者用PRF envelopeだけを受け取り、`folder-member`はcredential固有の一般ユーザー用vaultと選択linkのroot folder envelopeだけを受け取る。member専用roleや氏名・フォルダー名による認可例外は作らない。`subadmin`は従来ID/PWログイン用として維持し、パスキー候補・handoff・sessionから除外する。同一IdentityにCloud linkが1件なら自動選択し、複数なら選んだ1 linkだけのaccount／root scopeでsessionを発行して権限を合成しない。
...
T-Cloudの`admin`は`primary-admin`の既定linkに限定し、通常の追加APIは`folder-member`だけを受け付ける。`subadmin`はPWログイン用であり、旧パスキーlinkは無効化する。新しいT-Cloud folder linkの選択候補は削除されていないトップ階層フォルダだけとし、選択したトップフォルダの配下は従来どおり同じ連携範囲へ含める。過去に保存された配下フォルダlinkは範囲を変えず、表示・管理・失効を継続できる。第一管理者のT-Cloud admin・日記main-admin／main-user・請求書ownerは解除不能な基幹linkだが、同じIdentityへ通常の追加linkを付けられる。日記管理者や請求書owner等の特権accountを追加する場合は、直近5分以内のSecurity管理者パスキー認証をサーバー側で要求する。日記・請求書のactive Identityへのlinkは検証後すぐ有効化できるが、T-Cloud folder linkはcredential単位の鍵委譲が完了するまでpendingとする。
