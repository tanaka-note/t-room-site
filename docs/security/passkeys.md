...
パスキー由来の各サービスcookieにはIdentity、credential、service link、passkey session epochを識別する値を保持し、保護APIごとにSecurity Centerへ有効状態を照会する。credential無効化、service link解除、Identity停止、`PASSKEY_ENABLED=false`のいずれかが発生した場合は次の保護APIアクセスで失効する。解除済みservice link IDは再利用せず、同じアカウントを再連携しても古いcookieを復活させない。kill switchを解除しても発動前のepochを持つcookieは失効したままとする。古い形式で検証用IDを持たないパスキーcookieも再ログインを要求する。PW由来cookieはこの照会対象外である。
...
第一管理者は現在のPW由来account keyで既存RSA管理者秘密鍵PKCS#8を端末内復号し、WebAuthn PRF出力からHKDFで導出したAES-GCM鍵で再暗号化する。一般IdentityはcredentialごとにRSA-OAEP鍵ペアを1組だけ端末で生成し、秘密鍵を同じPRF由来鍵で暗号化する。第一管理者の端末が各folder keyを復号し、そのcredential共通の公開鍵へRSA-OAEPでwrapして承認する。公開鍵・暗号化済み秘密鍵はcredential単位、暗号化済みfolder keyはcredentialとservice linkの組単位で保存する。サーバー側に同じcredentialの公開鍵が存在する通常retryでは鍵を作り直さず、異なる鍵への置換は拒否する。
...
WebAuthn credential登録とT-Cloud鍵準備は別状態として扱う。PRF非対応または鍵envelope作成・通信が失敗しても、登録済みcredentialをSecurity・日記・請求書の「登録失敗」とは扱わない。T-Cloud linkだけを未準備のまま保つ。短期setup sessionにはIdentity・credential・進捗だけを保持し、ページ再読込後も同じcredentialによるuser verificationをやり直して安全に再開できる。PRF出力や秘密鍵はブラウザ保存領域へ平文保存しない。第一管理者もT-Cloud鍵envelope保存完了までCloud linkを有効化しない。
