...
test('five-day warning, seven-day refusal observation, updater/monitor stop and unknown are distinct', () => {
  ...
  assert.ok(definitionStatus({ ...good, monitor_checked_at: now - 3 * 3600 - 1 }, now).issues.includes('monitor_stopped'));
...
test('external monitor detects a stopped Worker monitor and status access failure', async () => {
  ...
  try {
    h.db.prepare('UPDATE security_definition_updates SET monitor_checked_at=?').run(now - 3 * 3600 - 1);
