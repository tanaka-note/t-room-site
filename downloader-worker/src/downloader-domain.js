...
const PERMANENT_DOWNLOAD_ERRORS = new Set([
  ...
  "scan_normalized_duration_mismatch", "scan_normalized_faststart_missing"
]);
export function isPermanentDownloadError(error) {
  return PERMANENT_DOWNLOAD_ERRORS.has(error?.code);
}
...
export function publicJob(row) {
  return {
    ...
    deletionConfirmed: !row.object_key && Boolean(row.deleted_at),
