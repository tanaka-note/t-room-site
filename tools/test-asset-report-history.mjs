...
assert.equal(summaryElements.get("#profit-value").textContent, "+￥289,484", "調整後の損益計算を維持する");
assert.equal(summaryElements.get("#return-value").textContent, "+4.82%", "調整後の損益率計算を維持する");
assert.equal(summaryElements.get("#market-value").textContent, "￥6,289,484", "投資信託他売却損を反映した時価総額を表示する");
assert.equal(portfolioTotal, 6_509_484, "資産構成のセグメント・構成比には従来の保有資産合計を使う");
assert.match(reportHtml, /<span>更新日<\/span>/);
assert.match(reportHtml, /<time id="report-updated" datetime="2026-09-09">2026\.09\.09<\/time>/);
...
assert.match(reportSource, /period: "2026-09-09"/);
...
assert.match(reportSource, /name: "ビットコイン"[\s\S]*?marketValue: 1977604/);
