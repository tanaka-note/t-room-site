...
CREATE TABLE security_service_links_next (
  ...
  service TEXT NOT NULL CHECK (service IN ('cloud', 'diary', 'billing', 'ai', 'downloader', 'downloader2')),
...
CREATE TABLE security_audit_events_next (
  ...
  service TEXT NOT NULL CHECK (service IN ('security', 'cloud', 'diary', 'billing', 'ai', 'downloader', 'downloader2')),
...
SELECT lower(hex(randomblob(16))), identity.id, 'downloader2', 'owner', NULL, 'T-lain Downloader 2 管理者',
...
WHERE identity.id = 'primary-admin'
  AND NOT EXISTS (
    ...
    WHERE link.identity_id = identity.id AND link.service = 'downloader2'
