const API = "/cloud/api";
const APP_BUILD_ID = "20260811-13";
...
async function saveSelectedOffline() {
  ...
  syncSelectionBar();
  await showOfflineProgressAtCloudTop();
  let completed = 0;
  const failures = [];
...
async function showOfflineProgressAtCloudTop() {
  if (state.folderId || state.view !== "all" || state.kind) {
    await navigateToFolder(null, "フォルダ");
  }
...
function addPreviewPlayerControls(stage, video) {
  ...
  const syncPlayback = () => {
    ...
    const bufferedPercent = duration ? Math.max(playedPercent, bufferedEnd(duration) / duration * 100) : 0;
