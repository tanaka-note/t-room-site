...
function addSharedPreviewSeekbarFullscreenControl(stage, video) {
  ...
  button.title = "全画面で表示";
  button.addEventListener("click", toggleSharedPreviewPlayerFullscreen);
  stage.append(button);
}
...
function handleSharedPreviewDoubleClick(event) {
  ...
  video.currentTime = Math.max(0, video.currentTime + (event.clientX >= bounds.left + bounds.width / 2 ? 10 : -10));
}
...
function renderVideoPlayer(stage, file, url, generation) {
  ...
  addSharedPreviewSeekbarFullscreenControl(stage, video);
