...
async function fetchAndDecryptChunk(entry, index, options = {}) {
  ...
  const task = loadAndDecryptChunk(entry, index);
  entry.decryptingChunks?.set(index, task);
  try {
    const plain = await task;
    rememberDecryptedChunk(entry, index, plain);
    if (entry.descriptor.offlineOnly && !options.prefetch && index + 1 < Number(entry.descriptor.chunkCount || 0)) {
      fetchAndDecryptChunk(entry, index + 1, { prefetch: true }).catch(() => {});
    }
    return plain;
  } finally {
...
async function loadAndDecryptChunk(entry, index) {
  ...
  if (file.storageId && self.TCloudOffline?.supported()) {
    ...
    if (cached) return new Uint8Array(await TRoomCrypto.decryptFileChunk(entry.fileKey, cached, index));
  }
  ...
  for (const delay of RETRY_DELAYS) {
    ...
    try {
      ...
      if (file.storageId && self.TCloudOffline?.supported()) {
        self.TCloudOffline.putChunk(file.storageId, index, envelope, { expectedBytes: end - start + 1 }).catch(() => {});
      }
