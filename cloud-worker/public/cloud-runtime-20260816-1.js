const API = "/cloud/api";
const APP_BUILD_ID = "cloud-4b8b5d6e43ca";
...
const PLAYBACK_CACHE_LIMIT_OPTIONS = [536870912, 1073741824, 2147483648, 3221225472];
...
const state = {
  ...
  itemLoadedFolderIds: new Set(),
  itemLoadedFileIds: new Set(),
  progressiveItemsLoading: false,
  displayCacheWriteTimer: 0,
  thumbnailLoadGeneration: 0,
  thumbnailLoadObserver: null,
  thumbnailLoadQueue: [],
  ...
  previewPlaybackMode: "off",
  previewOriginScrollX: 0,
  previewOriginScrollY: 0,
  previewTouchStart: null,
  previewTapCandidate: null,
  previewLastPointerType: "mouse",
  previewSuppressTapUntil: 0,
  previewDoubleTapSeekTimer: 0,
  previewDoubleTapSeekSequence: 0,
...
function bindEvents() {
  ...
  document.addEventListener("troom:before-auto-update", (event) => {
    const transferActive = state.uploading || state.activeFolderUploadOperationId || state.downloadActive
      || state.offlineActive || state.pendingSafetyUpload || state.filePickerActive;
    const interactiveViewOpen = Boolean(document.querySelector("dialog[open]")) || state.previewPlayer || state.previewPictureInPictureActive;
    if (transferActive || interactiveViewOpen) event.preventDefault();
  });
  ...
  for (const picker of [$("#file-input"), $("#folder-input")]) {
    picker.addEventListener("click", () => {
      state.filePickerActive = true;
      window.addEventListener("focus", () => window.setTimeout(finishFilePickerInteraction, 0), { once: true });
    });
    picker.addEventListener("cancel", finishFilePickerInteraction);
  }
  $("#file-input").addEventListener("change", (event) => {
    finishFilePickerInteraction();
  ...
  $("#preview-stage").addEventListener("pointerup", rememberPreviewPointerType);
  $("#preview-stage").addEventListener("click", handlePreviewVideoTap);
  $("#preview-stage").addEventListener("dblclick", suppressPreviewVideoDoubleClick);
  ...
  // 移動確定はフォームの既定動作に依存させない。
  // キャッシュ更新の前後で任意機能の要素が不足しても、この2つは先に登録する。
  $("#move-submit")?.addEventListener("click", moveSelectedItems);
...
function finishFilePickerInteraction() {
  state.filePickerActive = false;
  document.dispatchEvent(new Event("troom:auto-update-ready"));
}
...
async function updateInstalledApp() {
  ...
  if (state.uploading || state.activeFolderUploadOperationId || state.downloadActive || state.offlineActive || state.pendingSafetyUpload || state.filePickerActive) {
...
async function handleFileDrop(event) {
  ...
  try {
    ...
    revealDroppedUploadProgress();
  } catch (error) {
    setNotice(`ドロップしたデータの読み取りに失敗しました：${error.message}`, true);
  }
}

function revealDroppedUploadProgress() {
...
async function enterApp(session, password = "", accountKey = null) {
  ...
  $("#share-file-button").hidden = true;
  $("#share-folder-button").hidden = true;
...
function initializeNavigationHistory() {
  ...
  state.kind = String(saved?.kind || "");
  state.view = String(saved?.view || "all");
  state.query = String(saved?.query || "");
  state.listMode = Boolean(saved?.listMode);
  restoreFolderSortPreference(folderId);
  syncSearchInputs();
  setViewTitle(folderName);
  const entry = {
    ...navigationEntry(folderId, folderName),
    scrollX: Math.max(0, Number(saved?.scrollX) || 0),
    scrollY: Math.max(0, Number(saved?.scrollY) || 0),
    originType: ["folder", "file"].includes(saved?.originType) ? saved.originType : "",
    originId: Number.isInteger(Number(saved?.originId)) && Number(saved.originId) > 0 ? Number(saved.originId) : null,
    kind: state.kind,
    view: state.view,
    query: state.query,
...
function setViewTitle(value) {
  const title = String(value || "");
  $("#view-title").textContent = title;
  $("#view-title").title = title;
}

function rememberCurrentNavigationPosition(originType = "", originId = null) {
  if (!state.historyReady) return;
  const current = history.state?.tcloud
    ? history.state
    : navigationEntry(state.folderId, $("#view-title").textContent);
...
async function navigateToFolder(folderId, folderName, options = {}) {
  ...
  const { pushHistory = true, load = true, resetScroll = pushHistory, originType = "", originId = null, restoreEntry = null } = options;
  ...
  if (pushHistory && state.historyReady) rememberCurrentNavigationPosition(originType, originId);
  state.folderId = folderId ? Number(folderId) : null;
  state.kind = restoreEntry ? String(restoreEntry.kind || "") : "";
  state.view = restoreEntry ? String(restoreEntry.view || "all") : "all";
  state.query = restoreEntry ? String(restoreEntry.query || "") : "";
  if (restoreEntry) state.listMode = Boolean(restoreEntry.listMode);
  restoreFolderSortPreference(state.folderId);
  syncSearchInputs();
  setViewTitle(folderName || (state.folderId ? "ファイル" : "フォルダ"));
...
async function handleHistoryNavigation(event) {
  ...
  try {
    ...
    await restoreNavigationPosition(target);
    if (target.previewId) {
...
function navigationOriginSelector(entry) {
  if (entry?.originType === "folder" && Number(entry.originId) > 0) {
    return `.folder-card[data-folder-id="${Number(entry.originId)}"]`;
  }
  if (entry?.originType === "file" && Number(entry.originId) > 0) {
    return `.file-card[data-file-id="${Number(entry.originId)}"]`;
  }
  return "";
}

async function ensureNavigationOriginRendered(entry) {
  const selector = navigationOriginSelector(entry);
  if (!selector || $(selector)) return;
  for (let attempt = 0; attempt < 256 && !$(selector); attempt += 1) {
    const folderIndex = entry.originType === "folder"
      ? state.folders.findIndex((folder) => Number(folder.id) === Number(entry.originId))
      : -1;
    const fileIndex = entry.originType === "file"
      ? state.files.findIndex((file) => Number(file.id) === Number(entry.originId))
      : -1;
    if (folderIndex >= 0 || fileIndex >= 0) {
      const combinedIndex = folderIndex >= 0 ? folderIndex : state.folders.length + fileIndex;
      state.itemRenderLimit = Math.max(state.itemRenderLimit, combinedIndex + 1);
      renderItems();
      await nextRenderOpportunity();
      continue;
    }
    if (!state.progressiveItemsLoading) break;
    await loadNextItemPage();
  }
}

async function restoreNavigationPosition(entry) {
  if (!entry?.tcloud) return;
  await ensureNavigationOriginRendered(entry);
  const left = Math.max(0, Number(entry.scrollX) || 0);
...
function syncAvailableActions() {
  ...
  $("#logout-button").classList.toggle("icon-only", !showLogoutLabel);
  ...
  $$("#search-input, #floating-search-input").forEach((input) => {
    input.placeholder = insideFolder ? "このフォルダ以下を検索" : "すべてのフォルダを検索";
  });
...
async function loadItems() {
  ...
  state.itemNextFileOffset = null;
  state.itemPageLoading = false;
  state.itemLoadedFolderIds = new Set();
  state.itemLoadedFileIds = new Set();
  let renderedInitialItems = false;
  let renderedCachedItems = false;
  ...
  try {
    if (state.view === "trash") {
      ...
      const data = await api("/upload-history", { signal: itemLoadSignal });
      state.folders = [];
      state.files = [];
      ...
      const data = await api("/deletion-requests", { signal: itemLoadSignal });
      state.folders = [];
      state.files = [];
      state.history = [];
      ...
      const data = await api("/shares", { signal: itemLoadSignal });
      state.folders = [];
      state.files = [];
      state.history = [];
      state.requests = [];
      ...
      const serverSort = state.sortUsesTypeDefaults || state.sort === "name" ? "updated-desc" : `${state.sort}-${state.sortDirection}`;
      ...
      let nextFolderOffset = normalizeNextItemOffset(data.nextFolderOffset);
      let nextFileOffset = normalizeNextItemOffset(data.nextFileOffset);
      state.itemPageParams = params.toString();
      state.itemNextFolderOffset = nextFolderOffset;
      state.itemNextFileOffset = nextFileOffset;
      state.itemLoadedFolderIds = new Set(state.folders.map((folder) => Number(folder.id)));
      state.itemLoadedFileIds = new Set(state.files.map((file) => Number(file.id)));
      state.progressiveItemsLoading = nextFolderOffset != null || nextFileOffset != null;
      renderItems();
...
function nextRenderOpportunity() {
  return new Promise((resolve) => {
    const ready = () => requestAnimationFrame(() => resolve());
    if ("requestIdleCallback" in window) window.requestIdleCallback(ready, { timeout: 250 });
    else window.setTimeout(ready, 24);
  });
}

async function loadNextItemPage() {
  if (state.itemPageLoading || !state.progressiveItemsLoading || !state.itemPageParams) return;
  const generation = state.itemLoadGeneration;
  const signal = state.itemLoadController?.signal;
  ...
  try {
    ...
    const [folderPage, filePage] = await Promise.all([
      folderPageParams ? api(`/items?${folderPageParams}`, { signal }) : null,
      filePageParams ? api(`/items?${filePageParams}`, { signal }) : null
    ]);
    if (generation !== state.itemLoadGeneration || signal.aborted) return;
    await hydrateSearchFolderKeyRecords([
      ...(folderPage?.searchFolders || []),
      ...(filePage?.searchFolders || [])
    ]);
    const [pageFoldersRaw, pageFilesRaw] = await Promise.all([
      hydrateFolderRecords(folderPage?.folders || [], { preserveOrder: true }),
      hydrateFileRecords(filePage?.files || [], { preserveOrder: true })
    ]);
    if (generation !== state.itemLoadGeneration || signal.aborted) return;
    const pageFolders = pageFoldersRaw.filter((folder) => matchesActiveSearchFolder(folder)).filter((folder) => {
      const id = Number(folder.id);
      if (state.itemLoadedFolderIds.has(id)) return false;
      state.itemLoadedFolderIds.add(id);
      return true;
    });
    const pageFiles = pageFilesRaw.filter((file) => matchesActiveSearchFile(file)).filter((file) => {
      const id = Number(file.id);
      if (state.itemLoadedFileIds.has(id)) return false;
      state.itemLoadedFileIds.add(id);
      return true;
    });
    state.folders = finalizeHydratedFolders([...state.folders, ...pageFolders]);
    state.files = finalizeHydratedFiles([...state.files, ...pageFiles]);
    state.itemNextFolderOffset = normalizeNextItemOffset(folderPage?.nextFolderOffset);
    state.itemNextFileOffset = normalizeNextItemOffset(filePage?.nextFileOffset);
    state.progressiveItemsLoading = state.itemNextFolderOffset != null || state.itemNextFileOffset != null;
    state.itemPageLoading = false;
    if (state.sortUsesTypeDefaults && !state.query) {
      $("#content-grid .item-render-sentinel")?.remove();
      appendProgressiveItems(pageFolders, pageFiles);
      scheduleEncryptedThumbnailLoading();
      scheduleMissingMediaDurations();
      scheduleMissingVideoThumbnails();
      installItemRenderSentinel();
    } else {
      renderItems();
    }
    syncUnlockedTopFolderNames();
...
function displayListingCacheKey(params) {
  ...
  normalized.delete("pageSize");
  normalized.delete("offset");
  normalized.delete("foldersOnly");
  normalized.delete("filesOnly");
  normalized.delete("continuation");
  normalized.set("folderId", state.folderId ? String(state.folderId) : "root");
  normalized.set("sortDirection", state.sortDirection);
  normalized.set("sortUsesTypeDefaults", state.sortUsesTypeDefaults ? "1" : "0");
...
function renderCachedDisplayListing(cached) {
  ...
  state.canTrashCurrentFolderContents = Boolean(cached.canTrashCurrentFolderContents);
  state.itemLoadedFolderIds = new Set(state.folders.map((folder) => Number(folder.id)));
  state.itemLoadedFileIds = new Set(state.files.map((file) => Number(file.id)));
  state.progressiveItemsLoading = false;
...
function scheduleDisplayListingCacheWrite(cacheKey) {
  ...
  state.displayCacheWriteTimer = window.setTimeout(() => {
    const payload = {
      ...
      breadcrumbs: cacheSafeRecords(state.breadcrumbs),
      folderSummary: state.folderSummary,
      canTrashCurrentFolderContents: state.canTrashCurrentFolderContents,
      savedAt: Date.now()
    };
    TCloudDisplayCache.putListing(scope, cacheKey, payload).catch(() => {});
  }, 120);
}

function cacheSafeRecords(records) {
...
function normalizeNextItemOffset(value) {
...
function canShareFile(file) {
  if (!file || file.trashed || file.offlineOnly || Number(file.cryptoVersion) !== 1 || !file.fileKey) return false;
...
function canShareFolder(folder) {
  if (!folder || folder.trashed || Number(folder.cryptoVersion) !== 1) return false;
  if (state.session?.role === "admin") return true;
  return Boolean(state.session?.role === "subadmin"
    && folder.isUnlocked
...
function canOpenFolderSettings(folder) {
  return Boolean(state.session?.role === "admin" || canChangeFolderPassword(folder) || canTrashFolder(folder) || canShareFolder(folder));
}
...
function canRelockFolder(folder) {
  return Boolean(state.session?.role === "subadmin"
    && folder
    && folder.isProtected
    && folder.isUnlocked
...
function folderCard(folder) {
  ...
  button.type = "button";
  button.title = folder.name;
  button.setAttribute("aria-label", `フォルダ「${folder.name}」を開く`);
...
async function mapWithConcurrency(records, limit, mapper) {
...
async function hydrateFolderRecords(records, options = {}) {
  ...
  return finalizeHydratedFolders(hydrated);
}
...
async function openShareDialog(type, target) {
  if (!target || !canShareTarget(type, target)) {
    setNotice("PWで解除したフォルダ内のデータだけ共有できます。", true);
    return;
  }
  try {
    let targetKey = null;
...
async function createShare(event) {
  ...
  if (!target || !["admin", "subadmin"].includes(state.session?.role)) return;
...
async function deleteSelectedFolder() {
  ...
  try {
    ...
    setNotice(state.session?.canDelete
      ...
      : "削除しました。");
    invalidateStoredConflicts();
    preserveListingAfterDeletion({ folders: [folder] });
...
function fileCard(file) {
  ...
  card.classList.add(usesSquareFileCard(file) ? "media-file-card" : "non-media-file-card");
...
function installLongPressSelection(card, file) {
  ...
  card.addEventListener("pointermove", (event) => {
    ...
    if (!started && distance > 8) {
      stopTimer();
      return;
    }
    if (!started) return;
    if (!dragSelectionActive && distance < LONG_PRESS_DRAG_THRESHOLD_PX) return;
...
function resetEncryptedThumbnailLoading() {
  state.thumbnailLoadGeneration += 1;
...
function syncSelectionBar() {
  ...
  const canLockSelection = folderCount === 1 && fileCount === 0 && canRelockFolder(folders[0]);
  $("#selection-lock").hidden = !canLockSelection;
  $("#selection-lock").disabled = !canLockSelection || state.offlineActive;
  const canDownloadSelection = fileCount > 0 && folderCount === 0;
  $("#selection-download").hidden = !canDownloadSelection;
  $("#selection-download").disabled = !canDownloadSelection;
  const canSaveOffline = Boolean(fileCount > 0
    && folderCount === 0
    && files.every((file) => !file.trashed && Number(file.cryptoVersion) === 1 && file.fileKey)
    && offlineSelectionContext(files)
...
function preserveListingAfterDeletion({ files = [], folders = [] } = {}) {
  const deletedFileIds = new Set(files.map((file) => Number(file?.id)).filter(Number.isFinite));
  ...
  state.folders = state.folders.filter((folder) => !deletedFolderIds.has(Number(folder.id)));
  for (const id of deletedFileIds) state.itemLoadedFileIds.delete(id);
  for (const id of deletedFolderIds) state.itemLoadedFolderIds.delete(id);

  if (state.itemNextFileOffset != null) {
    state.itemNextFileOffset = Math.max(0, state.itemNextFileOffset - removedFiles.length);
  }
  if (state.itemNextFolderOffset != null) {
    state.itemNextFolderOffset = Math.max(0, state.itemNextFolderOffset - removedFolders.length);
  }
  if (state.folderSummary) {
    const removedFileBytes = removedFiles.reduce((sum, file) => sum + Number(file.sizeBytes || 0), 0);
    state.folderSummary = {
      ...state.folderSummary,
      fileCount: Math.max(0, Number(state.folderSummary.fileCount || 0) - removedFiles.length),
      folderCount: Math.max(0, Number(state.folderSummary.folderCount || 0) - removedFolders.length),
      totalFileCount: Math.max(0, Number(state.folderSummary.totalFileCount || 0) - removedFiles.length),
      totalSizeBytes: Math.max(0, Number(state.folderSummary.totalSizeBytes || 0) - removedFileBytes)
    };
  }
...
function offlineSelectionContext(files = []) {
  const context = currentOfflineContext();
  if (!context || !state.folderId || !files.length) return null;
  const currentFolderId = Number(state.folderId);
  const currentFolderKey = state.crypto.folderKeys.get(currentFolderId);
  if (!(currentFolderKey instanceof CryptoKey)) return null;
  const protectedPathUnlocked = state.breadcrumbs.every((folder) => !folder.isProtected
    || (folder.isUnlocked && state.crypto.folderKeys.get(Number(folder.id)) instanceof CryptoKey));
  if (!protectedPathUnlocked) return null;
  if (files.some((file) => file.trashed || Number(file.folderId) !== currentFolderId || !(file.fileKey instanceof CryptoKey))) return null;
  return context;
}
...
async function saveSelectedOffline() {
  ...
  syncSelectionBar();
  await showOfflineProgressInCurrentFolder();
  let completed = 0;
  const failures = [];
...
async function showOfflineProgressInCurrentFolder() {
  await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
  $("#offline-panel").scrollIntoView({ behavior: "smooth", block: "center" });
}
...
async function openMoveDialog() {
  ...
  $("#move-current-location").textContent = "読み込み中…";
  $("#move-breadcrumbs").innerHTML = "";
  $("#move-folder-list").innerHTML = '<p class="move-picker-loading">フォルダを読み込んでいます…</p>';
  $("#move-folder-empty").hidden = true;
  $("#move-picker-up").disabled = true;
  submit.disabled = false;
  submit.textContent = "移動先を読み込み中…";
  ...
  try {
    ...
    state.movePicker = buildMovePicker(data.folders || [], scopeRoot, selectedFolderIds, files, folders);
    renderMovePicker();
  } catch (error) {
    state.movePicker = null;
    $("#move-folder-list").innerHTML = "";
    $("#move-folder-empty").textContent = "移動先を読み込めませんでした。";
    $("#move-folder-empty").hidden = false;
    $("#move-current-location").textContent = "読み込みに失敗しました";
    $("#move-error").textContent = error.message;
  }
}

function buildMovePicker(records, scopeRoot, selectedFolderIds, files, selectedFolders) {
  ...
  const isExcluded = (folder) => {
    ...
    while (current && guard++ < 100) {
      if (selectedFolderIds.has(Number(current.id))) return true;
      current = current.parentId == null ? null : byId.get(Number(current.parentId));
    }
    return false;
  };
  const allowed = folders.filter((folder) => !isExcluded(folder));
  const allowedIds = new Set(allowed.map((folder) => folder.id));
  const children = new Map();
  for (const folder of allowed) {
    const parentId = folder.parentId == null || !allowedIds.has(folder.parentId) ? null : folder.parentId;
  ...
  const scopeRootId = scopeRoot ? Number(scopeRoot.id) : null;
  if (scopeRootId && !allowedIds.has(scopeRootId)) throw new Error("移動可能なフォルダを確認できませんでした。");
  state.moveDestinations.clear();
  for (const folder of allowed) state.moveDestinations.set(folder.id, folder);
  return {
    byId: state.moveDestinations,
    children,
    currentId: scopeRootId,
    scopeRootId,
    files,
...
function movePickerPath(picker) {
  if (!picker?.currentId) return [];
  const path = [];
  let current = picker.byId.get(Number(picker.currentId));
  let guard = 0;
  while (current && guard++ < 100) {
    path.unshift(current);
    if (Number(current.id) === Number(picker.scopeRootId)) break;
    current = current.parentId == null ? null : picker.byId.get(Number(current.parentId));
  }
  return path;
}

function movePickerCurrentIsSource(picker) {
  const destinationId = picker.currentId == null ? null : Number(picker.currentId);
  const sourceParents = [
    ...picker.files.map((file) => Number(file.folderId)),
    ...picker.folders.map((folder) => folder.parentId == null ? null : Number(folder.parentId))
  ];
  return sourceParents.length > 0 && sourceParents.every((parentId) => parentId === destinationId);
}

function renderMovePicker() {
  const picker = state.movePicker;
  if (!picker) return;
  const current = picker.currentId == null ? null : picker.byId.get(Number(picker.currentId));
  const currentName = current?.name || "Cloud Storage（最上位）";
  $("#move-current-location").textContent = currentName;
  $("#move-destination").value = current ? String(current.id) : "root";

  const breadcrumbs = $("#move-breadcrumbs");
  breadcrumbs.innerHTML = "";
  const rootButton = document.createElement("button");
  rootButton.type = "button";
  rootButton.textContent = picker.scopeRootId ? picker.byId.get(picker.scopeRootId)?.name || "移動可能なフォルダ" : "Cloud Storage";
  rootButton.dataset.folderId = picker.scopeRootId == null ? "root" : String(picker.scopeRootId);
  rootButton.setAttribute("aria-current", Number(picker.currentId) === Number(picker.scopeRootId) ? "location" : "false");
  rootButton.addEventListener("click", () => openMovePickerFolder(picker.scopeRootId));
  breadcrumbs.append(rootButton);
  const path = movePickerPath(picker);
  const visiblePath = picker.scopeRootId ? path.slice(1) : path;
  for (const folder of visiblePath) {
    breadcrumbs.append(Object.assign(document.createElement("span"), { textContent: "/" }));
    const button = document.createElement("button");
    button.type = "button";
    button.textContent = folder.name;
    button.dataset.folderId = String(folder.id);
    button.setAttribute("aria-current", Number(folder.id) === Number(picker.currentId) ? "location" : "false");
    button.addEventListener("click", () => openMovePickerFolder(folder.id));
    breadcrumbs.append(button);
  }
  requestAnimationFrame(() => { breadcrumbs.scrollLeft = breadcrumbs.scrollWidth; });

  const list = $("#move-folder-list");
  list.innerHTML = "";
  const childFolders = picker.children.get(picker.currentId == null ? null : Number(picker.currentId)) || [];
  ...
  $("#move-folder-empty").textContent = "この中にフォルダはありません。";
  $("#move-folder-empty").hidden = childFolders.length > 0;

  const atScopeRoot = picker.scopeRootId != null && Number(picker.currentId) === Number(picker.scopeRootId);
  $("#move-picker-up").disabled = picker.currentId == null || atScopeRoot;
  const filesCannotUseRoot = picker.files.length > 0 && picker.currentId == null;
  const unprotectedFolderCannotUseRoot = picker.currentId == null && picker.folders.some((folder) => !folder.isProtected);
  const sameLocation = movePickerCurrentIsSource(picker);
  const submit = $("#move-submit");
  // Firefoxを含め、無効なボタンが「反応しない」ように見えないようにする。
  // 同じ場所の場合も押せる状態を保ち、押した時に理由を明示する。
  submit.disabled = false;
  submit.textContent = sameLocation ? "移動先を変更" : "ここへ移動";
  $("#move-error").textContent = filesCannotUseRoot
    ? "ファイルはフォルダ内へ移動してください。"
    : unprotectedFolderCannotUseRoot
      ? "PWを持たないフォルダは最上位へ移動できません。"
      : sameLocation ? "移動元と同じ場所です。" : "";
}

function openMovePickerFolder(folderId) {
  if (!state.movePicker) return;
  const normalizedId = folderId == null ? null : Number(folderId);
  if (normalizedId != null && !state.movePicker.byId.has(normalizedId)) return;
  state.movePicker.currentId = normalizedId;
  renderMovePicker();
}

function movePickerUp() {
  const picker = state.movePicker;
  if (!picker || picker.currentId == null) return;
  const current = picker.byId.get(Number(picker.currentId));
  const parentId = current?.parentId == null ? null : Number(current.parentId);
  if (picker.scopeRootId != null && Number(picker.currentId) === Number(picker.scopeRootId)) return;
  openMovePickerFolder(parentId);
}

async function loadMoveDestination(folderId) {
  // foldersOnly=1 は一覧の高速化用で、breadcrumbs を返さない。
  // 移動時は暗号鍵の再包装に階層情報が必要なため、最小ページの通常取得を使う。
  const data = await api(`/items?folderId=${Number(folderId)}&pageSize=1`);
...
async function moveSelectedItems(event) {
  ...
  if (movePickerCurrentIsSource(picker)) {
    $("#move-error").textContent = "現在と同じ場所です。別のフォルダを選択してください。";
    return;
  }
  const destinationId = picker.currentId == null ? null : Number(picker.currentId);
  ...
  if (!destinationId && folders.some((folder) => !folder.isProtected)) {
    $("#move-error").textContent = "PWを持たないフォルダは最上位へ移動できません。";
    return;
  }
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        failed += 1;
        failures.push({ name: file.name, message: error.message || "移動できませんでした。" });
      }
    }
    for (const folder of folders) {
      try {
        ...
        failed += 1;
        failures.push({ name: folder.name, message: error.message || "移動できませんでした。" });
      }
    }
    if (!completed && failures.length) {
      throw new Error(`${failures[0].name}：${failures[0].message}`);
    }
    $("#move-dialog").close();
    clearFileSelection();
    const failedNames = failures.slice(0, 3).map((failure) => failure.name).join("、");
    setNotice(failed
      ? `${completed}件を移動しました。${failed}件は移動できませんでした：${failedNames}${failures.length > 3 ? " ほか" : ""}`
      : `${completed}件を移動しました。`, Boolean(failed));
    if (completed) invalidateStoredConflicts();
    await loadItems();
  } catch (error) {
    fatalError = error;
    ...
    submit.disabled = false;
    submit.textContent = "ここへ移動";
    if ($("#move-dialog").open) {
      renderMovePicker();
      if (fatalError) $("#move-error").textContent = fatalError.message || "移動できませんでした。";
    }
  }
}
...
async function uploadOne(file, index, total, destinationFolderId, destinationFolderKey, signal, partLimiter, tracker, safetyConfirmed = false) {
  ...
  const inspection = safetyConfirmed ? null : await TCloudSafety.inspect(file);
  throwIfUploadCancelled(signal);
  ...
  try {
    ...
    init = await api("/uploads", {
      method: "POST",
      body: JSON.stringify({ ...encrypted.payload, folderId, fastDisplay }),
      signal
    });
...
function createFastDisplayMetadata(file, mediaKind, inspection, durationSeconds = null) {
  const signature = String(inspection?.signature || "").toLowerCase();
  const allowed = {
    image: new Set(["jpeg", "png", "gif", "webp"]),
    audio: new Set(["wav", "ogg", "flac", "mpeg-audio"]),
    document: new Set(["pdf", "zip"])
  };
  if (mediaKind === "video" || !allowed[mediaKind]?.has(signature)) return null;
  const metadata = {
    name: file.name,
...
function resetBackgroundMediaWork() {
  ...
  state.thumbnailBackfillScheduled = false;
  for (const token of state.backgroundMediaTokens) {
    try { TCloudMedia.releaseMedia(token); } catch {}
  }
  state.backgroundMediaTokens.clear();
}
...
async function registerMediaWithDeviceCache(file, endpoint) {
  if (!file.offlineOnly) attachOfflineStorageIdentity(file);
...
function scheduleMissingVideoThumbnails() {
  ...
  if (!eligible.length) return;
  const enqueue = (file) => {
    if (generation !== state.itemLoadGeneration || state.thumbnailAttempts.has(Number(file.id))) return;
    const queued = state.thumbnailBackfillQueue.some((item) => Number(item.id) === Number(file.id));
    if (!queued) state.thumbnailBackfillQueue.push(file);
    scheduleVideoThumbnailBackfill(generation);
  };
  if (!globalThis.IntersectionObserver) {
    eligible.slice(0, 2).forEach(enqueue);
    return;
  }
  ...
  const observer = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      if (!entry.isIntersecting) continue;
      observer.unobserve(entry.target);
      const file = filesByCard.get(entry.target);
      if (file) enqueue(file);
    }
  }, { rootMargin: "360px 0px" });
  state.thumbnailBackfillObserver = observer;
  for (const file of eligible) {
    const card = $(`.file-card[data-file-id="${Number(file.id)}"]`);
    if (!card) continue;
    filesByCard.set(card, file);
    observer.observe(card);
  }
}

function scheduleVideoThumbnailBackfill(generation) {
  if (state.thumbnailBackfillRunning || state.thumbnailBackfillScheduled) return;
  state.thumbnailBackfillScheduled = true;
  const run = () => {
    state.thumbnailBackfillScheduled = false;
    if (generation !== state.itemLoadGeneration || state.view !== "all") return;
    void backfillMissingVideoThumbnails(generation);
  };
  if ("requestIdleCallback" in window) window.requestIdleCallback(run, { timeout: 5000 });
  else window.setTimeout(run, 900);
}

async function backfillMissingVideoThumbnails(generation) {
...
async function unlockFolder(event) {
  ...
  try {
    ...
    $("#unlock-dialog").close();
    invalidateStoredConflicts();
    await navigateToFolder(id, folder.name, { originType: "folder", originId: id });
  } catch (error) {
...
async function openPreview(file, options = {}) {
  ...
  if (Number(file?.cryptoVersion) === 1 && !file.fileKey && !file.offlineOnly) {
    try {
      const data = await api(`/files/${Number(file.id)}`);
      const [current] = await hydrateFileRecords([data.file], { preserveOrder: true });
      if (current?.fileKey) {
        const position = state.files.findIndex((item) => Number(item.id) === Number(current.id));
        if (position >= 0) state.files[position] = current;
        file = current;
      }
    } catch (error) {
      setNotice(error.message, true);
  ...
  if (file.mediaKind === "image") {
    ...
    renderAudioPlayer(stage, file, url);
...
async function navigatePreview(direction) {
  ...
  let index = images.findIndex((file) => Number(file.id) === Number(state.previewFileId));
  let next = images[index + direction];
  if (!next && direction > 0 && state.progressiveItemsLoading) {
    await loadNextItemPage();
    images = previewImages();
    index = images.findIndex((file) => Number(file.id) === Number(state.previewFileId));
    next = images[index + direction];
  }
  if (!next) return;
...
function previewAudioFiles() {
  return state.files.filter((file) => !file.trashed && file.mediaKind === "audio");
}

async function navigatePreviewAudio(direction = 1) {
  if (!$("#preview-dialog").open) return false;
  let audioFiles = previewAudioFiles();
  let index = audioFiles.findIndex((file) => Number(file.id) === Number(state.previewFileId));
  let next = audioFiles[index + direction];
  if (!next && direction > 0 && state.progressiveItemsLoading) {
    await loadNextItemPage();
    audioFiles = previewAudioFiles();
    index = audioFiles.findIndex((file) => Number(file.id) === Number(state.previewFileId));
    next = audioFiles[index + direction];
  }
  if (!next) return false;
  await openPreview(next, { pushHistory: false });
  const media = $("#preview-stage audio");
  if (media) await media.play().catch(() => {});
  return true;
}

function nextPreviewPlaybackMode(mediaKind) {
  if (mediaKind === "audio") {
    if (state.previewPlaybackMode === "off") return "repeat-one";
    if (state.previewPlaybackMode === "repeat-one") return "continuous-audio";
    return "off";
  }
  return state.previewPlaybackMode === "repeat-one" ? "off" : "repeat-one";
}

function previewPlaybackModeLabel(mediaKind) {
  if (state.previewPlaybackMode === "repeat-one") return mediaKind === "audio" ? "1曲リピート" : "動画リピート";
  if (state.previewPlaybackMode === "continuous-audio") return "連続再生";
  return "リピート：オフ";
}

function syncPreviewPlaybackModeButton(button, media, mediaKind) {
  if (!button || !media) return;
  if (mediaKind !== "audio" && state.previewPlaybackMode === "continuous-audio") state.previewPlaybackMode = "off";
...
function bindPreviewPlaybackMode(button, media, file) {
  syncPreviewPlaybackModeButton(button, media, file.mediaKind);
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    state.previewPlaybackMode = nextPreviewPlaybackMode(file.mediaKind);
    syncPreviewPlaybackModeButton(button, media, file.mediaKind);
  });
  if (file.mediaKind === "audio") {
    media.addEventListener("ended", () => {
      if (state.previewPlaybackMode === "continuous-audio") void navigatePreviewAudio(1);
    });
  }
}

function renderAudioPlayer(stage, file, url) {
  const wrapper = document.createElement("div");
  wrapper.className = "preview-audio-player";
  const audio = document.createElement("audio");
  audio.controls = true;
  audio.preload = "metadata";
  observeAndPersistMediaDuration(audio, file);
  const modeButton = document.createElement("button");
  modeButton.type = "button";
  modeButton.className = "preview-playback-mode preview-audio-playback-mode";
  bindPreviewPlaybackMode(modeButton, audio, file);
  audio.src = url;
  wrapper.append(audio, modeButton);
...
function previewVideoTapAction(pointerType, isDoubleTap) {
  if (!isDoubleTap) return "toggle";
  return pointerType === "touch" || pointerType === "pen" ? "seek" : "none";
}

function rememberPreviewPointerType(event) {
  state.previewLastPointerType = event.pointerType || "mouse";
}

function togglePreviewVideoPlayback(video) {
  if (video.paused) video.play().catch(() => {});
  else video.pause();
}

function seekPreviewVideoBy(video, seconds) {
  ...
  video.dispatchEvent(new Event("tcloud:seek-feedback"));
}

function holdPreviewControlsForDoubleTapSeek(stage) {
  if (state.previewDoubleTapSeekTimer) clearTimeout(state.previewDoubleTapSeekTimer);
  const releaseToken = ++state.previewDoubleTapSeekSequence;
  stage.classList.add("is-double-tap-seeking");
  state.previewDoubleTapSeekTimer = setTimeout(() => {
    if (releaseToken !== state.previewDoubleTapSeekSequence) return;
    state.previewDoubleTapSeekTimer = 0;
    stage.classList.remove("is-double-tap-seeking");
  }, DOUBLE_TAP_SEEK_CONTROLS_HOLD_MS);
}

function handlePreviewVideoTap(event) {
  const stage = $("#preview-stage");
  const video = stage.querySelector("video");
  if (!video || event.button !== 0) return;
  if (performance.now() < state.previewSuppressTapUntil) return;
  const bounds = stage.getBoundingClientRect();
  const pointerType = state.previewLastPointerType || (event.sourceCapabilities?.firesTouchEvents ? "touch" : "mouse");
  const now = performance.now();
  const side = event.clientX < bounds.left + bounds.width / 2 ? -1 : 1;
  const previous = state.previewTapCandidate;
  const isDoubleTap = Boolean(previous
    && now - previous.at <= 360
    && previous.side === side
    && Math.abs(previous.x - event.clientX) <= 96
    && Math.abs(previous.y - event.clientY) <= 96);
  if (previous) clearTimeout(previous.timer);
  const action = previewVideoTapAction(pointerType, isDoubleTap);
  if (isDoubleTap) {
    state.previewTapCandidate = null;
    if (action === "seek") {
      holdPreviewControlsForDoubleTapSeek(stage);
      seekPreviewVideoBy(video, side * DOUBLE_TAP_SEEK_SECONDS);
    }
    return;
  }
  const token = Symbol("preview-tap");
  const timer = setTimeout(() => {
    if (state.previewTapCandidate?.token !== token) return;
    state.previewTapCandidate = null;
    if (stage.querySelector("video") === video) togglePreviewVideoPlayback(video);
  }, 280);
  state.previewTapCandidate = { token, timer, at: now, x: event.clientX, y: event.clientY, side, pointerType };
}

function suppressPreviewVideoDoubleClick(event) {
  if (!$("#preview-stage video")) return;
  event.preventDefault();
}

function clearPreviewTapGesture() {
  if (state.previewTapCandidate?.timer) clearTimeout(state.previewTapCandidate.timer);
  if (state.previewDoubleTapSeekTimer) clearTimeout(state.previewDoubleTapSeekTimer);
  state.previewDoubleTapSeekSequence += 1;
  state.previewTapCandidate = null;
  state.previewLastPointerType = "mouse";
  state.previewDoubleTapSeekTimer = 0;
  $("#preview-stage")?.classList.remove("is-double-tap-seeking");
}
...
function handlePreviewTouchEnd(event) {
  ...
  state.previewSuppressTapUntil = performance.now() + 700;
...
function clearPreviewUrl() {
  ...
  state.previewPictureInPictureVideo = null;
  state.previewKeepMediaOnClose = false;
...
function shouldPausePreviewVideo(pageHidden, pictureInPictureActive) {
  return Boolean(pageHidden && !pictureInPictureActive);
}

function previewVideoIsInPictureInPicture(video) {
  if (!video) return false;
  return document.pictureInPictureElement === video
    || video.webkitPresentationMode === "picture-in-picture"
    || (state.previewPictureInPictureActive && state.previewPictureInPictureVideo === video);
}

function handlePreviewBackgroundVisibility(event) {
  const pageHidden = document.hidden || event?.type === "pagehide" || event?.type === "freeze";
  const video = state.previewPictureInPictureVideo || $("#preview-stage video");
  if (!video || !shouldPausePreviewVideo(pageHidden, previewVideoIsInPictureInPicture(video))) return;
...
function prepareVideoPlayer(stage, file) {
  ...
  const enablePictureInPicture = () => {
    if (Number(state.previewFileId) === Number(file.id)) $("#preview-pip").disabled = false;
  };
  video.addEventListener("loadedmetadata", enablePictureInPicture, { once: true });
  video.addEventListener("canplay", enablePictureInPicture, { once: true });
...
async function minimizePreviewToPictureInPicture() {
  ...
  const video = $("#preview-stage video");
  if (!dialog.open || !video) {
    setNotice("動画の再生準備が完了してから縮小してください。", true);
    return;
  }
  const originFileId = Number(state.previewFileId);
  ...
  try {
    ...
    if (document.pictureInPictureEnabled && typeof video.requestPictureInPicture === "function") {
      ...
      throw new Error("この端末またはブラウザはピクチャインピクチャに対応していません。");
    }
    state.previewPictureInPictureActive = true;
    state.previewPictureInPictureVideo = video;
    if (mode === "standard") {
      video.addEventListener("leavepictureinpicture", () => finishPictureInPicturePreview(video), { once: true });
    } else {
      video.addEventListener("webkitpresentationmodechanged", () => {
        if (video.webkitPresentationMode !== "picture-in-picture") finishPictureInPicturePreview(video);
      });
    }
    state.previewKeepMediaOnClose = true;
    state.previewHistoryActive = false;
    ...
    dialog.close();
    restorePreviewOrigin(originFileId);
  } catch (error) {
    const message = String(error?.message || "");
    setNotice(message.startsWith("この端末またはブラウザ")
      ? message
      : "この端末またはブラウザでは、現在ピクチャインピクチャを開始できません。", true);
  } finally {
    button.textContent = originalText;
    if (!state.previewPictureInPictureActive) button.disabled = false;
  }
}

function finishPictureInPicturePreview(video) {
  if (state.previewPictureInPictureVideo !== video) return;
  if (document.hidden) {
    try { video.pause(); } catch {}
  }
  state.previewPictureInPictureActive = false;
  ...
  state.previewGeneration += 1;
  clearPreviewUrl();
  restoreInstalledAppPortrait({ settle: true, reason: "preview-pip-exit" });
  state.previewFileId = null;
  state.previewTouchStart = null;
}

async function stopPictureInPicturePreview() {
  const video = state.previewPictureInPictureVideo;
  if (!video) return;
  try {
  ...
  finishPictureInPicturePreview(video);
}
...
function loadVideoPlayerSource(prepared, file, url, generation) {
  ...
  if (mpegType && globalThis.mpegts?.isSupported()) {
    ...
    const player = mpegts.createPlayer({ type: mpegType, isLive: false, url, filesize: Number(file.sizeBytes) }, {
      ...
      lazyLoadRecoverDuration: offlinePlayback ? 120 : 30,
      enableStashBuffer: true,
      stashInitialSize: offlinePlayback ? 2 * 1024 * 1024 : 1024 * 1024,
      accurateSeek: true,
      seekType: "range"
    });
...
async function persistMediaDuration(file, durationSeconds) {
  ...
  try {
    ...
    await api(`/files/${file.id}`, { method: "PATCH", body: JSON.stringify({ ...encryptedMetadata, fastDisplay: fastDisplayForExistingFile(file, file.name, normalized) }) });
...
function fastDisplayForExistingFile(file, name = file.name, durationSeconds = file.durationSeconds) {
  if (Number(file?.displayMetadataVersion || 0) !== 1 || file?.mediaKind === "video") return undefined;
  const metadata = {
    name,
    mimeType: file.mimeType || "application/octet-stream",
    mediaKind: file.mediaKind,
    lastModified: Number(file.lastModified || file.displayLastModified || 0)
  };
...
async function saveFile(event) {
  ...
  try {
    ...
    if (Number(file.cryptoVersion) === 1) {
      ...
      await api(`/files/${id}`, { method: "PATCH", body: JSON.stringify({ ...encryptedMetadata, fastDisplay: fastDisplayForExistingFile(file, name) }) });
      file.name = metadata.name;
      file.encryptedMetadata = encryptedMetadata.encryptedMetadata;
      file.metadataIv = encryptedMetadata.metadataIv;
    } else {
...
async function deleteSelectedFile() {
  ...
  try {
    ...
    invalidateStoredConflicts();
    preserveListingAfterDeletion({ files: [file] });
    if (state.session?.role === "admin") await loadUsage();
  } catch (error) { setNotice(error.message, true); }
}
...
async function executeBatchRename() {
  ...
  try {
    for (let index = 0; index < plan.length; index++) {
      ...
      prepared.push({ id: Number(operation.file.id), ...encrypted, fastDisplay: fastDisplayForExistingFile(operation.file, operation.after) });
...
async function refreshDeviceStorageSummary() {
  ...
  try {
    ...
    const cacheLimit = applyPlaybackCacheLimit();
    $("#device-cache-usage").textContent = `${formatBytes(summary.cacheBytes)} / 最大${formatBytes(cacheLimit)}`;
...
function selectedPlaybackCacheLimit() {
  const stored = Number(localStorage.getItem(playbackCacheLimitStorageKey()));
  return PLAYBACK_CACHE_LIMIT_OPTIONS.includes(stored) ? stored : 1024 * 1024 * 1024;
}

function applyPlaybackCacheLimit() {
  const selected = selectedPlaybackCacheLimit();
  const applied = globalThis.TCloudOffline?.setCacheLimitBytes?.(selected) || selected;
  globalThis.TCloudMedia?.setCacheLimitBytes?.(applied)?.catch?.(() => {});
  if ($("#device-cache-limit")) $("#device-cache-limit").value = String(applied);
  return applied;
}

async function changePlaybackCacheLimit(event) {
  const requested = Number(event.currentTarget.value);
  if (!PLAYBACK_CACHE_LIMIT_OPTIONS.includes(requested)) return;
  localStorage.setItem(playbackCacheLimitStorageKey(), String(requested));
  const applied = applyPlaybackCacheLimit();
  const context = currentOfflineContext();
  if (context) await TCloudOffline.enforceCacheLimit("", -1, context.accountScope);
  await refreshDeviceStorageSummary();
  setNotice(`この端末の再生キャッシュ上限を${formatBytes(applied)}に変更しました。`);
}
...
function createOfflineManagerItem(item) {
  ...
  const openLabel = ["video", "audio"].includes(item.file.mediaKind) ? "オフライン再生" : "開く";
  actions.innerHTML = `<span class="offline-manager-expiry">${escapeHtml(formatOfflineExpiry(item.entry.expiresAt))}まで</span><button class="secondary-button offline-open-button" type="button" data-storage-id="${escapeHtml(item.entry.id)}">${openLabel}</button>`;
...
async function handleOfflineManagerClick(event) {
  ...
  const row = event.target.closest(".offline-manager-item");
  if (!row || (!button && event.target.closest("input, button"))) return;
  const storageId = button?.dataset.storageId || row.dataset.storageId;
  const item = state.offlineManagerEntries.find((candidate) => candidate.entry.id === storageId);
  if (!item) return;
...
function scheduleUsageLoad() {
  if (state.session?.role !== "admin" || state.usageLoadScheduled) return;
  state.usageLoadScheduled = true;
  const run = () => {
    state.usageLoadScheduled = false;
    if (state.session?.role === "admin") void loadUsage();
  };
  if ("requestIdleCallback" in window) window.requestIdleCallback(run, { timeout: 3000 });
...
function renderUsage(usage) {
  ...
  $("#file-count").textContent = `${Number(usage.activeFileCount || 0).toLocaleString("ja-JP")}ファイル`;
...
function previewDateDetails(file) {
  const editableDocument = /\.(?:doc|docx|docm|dot|dotx|xls|xlsx|xlsm|xlsb|ods|odt|ppt|pptx|pptm|odp|rtf|txt|csv|tsv)$/i.test(String(file?.name || ""))
    || /(?:msword|wordprocessingml|excel|spreadsheetml|powerpoint|presentationml|opendocument\.(?:text|spreadsheet|presentation)|text\/(?:plain|csv|tab-separated-values))/i.test(String(file?.mimeType || ""));
  return editableDocument
    ? { label: "更新日", value: file?.updatedAt || file?.createdAt }
    : { label: "保存日", value: file?.createdAt };
}
