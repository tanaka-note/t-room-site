...
const PREVIEW_FULLSCREEN_PORTRAIT_HOLD_MS = 600;
let previewOrientationReleaseTimer = 0;
const state = {
  session: null,
  loginId: "",
  folderId: null,
...
async function updateInstalledApp() {
  ...
  if (state.uploading || state.activeFolderUploadOperationId || state.downloadActive) {
  ...
  try {
    const registration = await navigator.serviceWorker.getRegistration("/cloud/");
    if (registration) await registration.update();
    await fetch(`/cloud/?app-update=${Date.now()}`, {
      cache: "no-store",
      credentials: "same-origin"
    });
    setNotice("最新版を取得しました。ログイン状態を維持したまま画面を更新します。");
...
async function planFolderUpload(selection, baseParentId, baseParentKey, operationId) {
  ...
  for (const file of selection.looseFiles || []) {
    ...
    precheckDestinations.set(file, { displayName: file.name });
  }
...
function findIncomingUploadConflictGroups(files) {
  const groups = new Map();
  for (const file of files) {
    const identity = uploadFileIdentity(file.name, file.size);
...
async function excludeExistingUploadFiles(files, destinations) {
  ...
  for (const group of findIncomingUploadConflictGroups(files)) {
  ...
  if (remaining.length) {
    ...
    const { candidates, folders } = await loadUploadConflictCandidates(remaining.map((file) => file.size));
...
function clearPreviewOrientationReleaseTimer() {
  if (!previewOrientationReleaseTimer) return;
  clearTimeout(previewOrientationReleaseTimer);
  previewOrientationReleaseTimer = 0;
}

async function restoreInstalledAppPortrait() {
  if (!isInstalledAppMode() || !screen.orientation?.lock) return;
  clearPreviewOrientationReleaseTimer();
...
async function prepareInstalledVideoFullscreen() {
  ...
  clearPreviewOrientationReleaseTimer();
  try { await screen.orientation.lock("portrait-primary"); } catch {}
  previewOrientationReleaseTimer = window.setTimeout(() => {
    previewOrientationReleaseTimer = 0;
    try { screen.orientation?.unlock?.(); } catch {}
  }, PREVIEW_FULLSCREEN_PORTRAIT_HOLD_MS);
}
...
function syncPreviewSeekbarFullscreenControl(fullscreenElement = document.fullscreenElement || document.webkitFullscreenElement) {
  const button = $("#preview-stage .preview-seekbar-fullscreen");
  if (!button) return;
...
function addPreviewSeekbarFullscreenControl(stage, video) {
  if (video.controlsList?.add) video.controlsList.add("nofullscreen");
  else video.setAttribute("controlsList", "nofullscreen");
  const button = document.createElement("button");
  button.type = "button";
  button.className = "preview-seekbar-fullscreen";
  button.textContent = "⛶";
  button.setAttribute("aria-label", "全画面で表示");
  button.title = "全画面で表示";
  button.addEventListener("click", togglePreviewPlayerFullscreen);
  stage.append(button);
}
...
function renderVideoPlayer(stage, file, url, generation) {
  ...
  addPreviewSeekbarFullscreenControl(stage, video);
