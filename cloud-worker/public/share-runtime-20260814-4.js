...
function bindEvents() {
  ...
  $("#preview-stage").addEventListener("pointerup", rememberSharedPreviewPointerType);
  $("#preview-stage").addEventListener("click", handleSharedPreviewVideoTap);
  $("#preview-stage").addEventListener("dblclick", suppressSharedPreviewVideoDoubleClick);
  ...
  document.addEventListener("webkitfullscreenchange", handleSharedPreviewFullscreenOrientationChange);
  window.addEventListener("pageshow", enforceSharedFolderPortraitOrientation);
  window.addEventListener("orientationchange", enforceSharedFolderPortraitOrientation);
...
async function openPreview(file, options = {}) {
  ...
  $("#share-preview-date-label").textContent = previewDate.label;
  $("#share-preview-date").textContent = formatDateTime(previewDate.value);
  const stage = $("#preview-stage");
  stage.classList.remove("has-custom-video-controls", "is-media-ready");
  let preparedVideo = null;
  if (file.mediaKind === "video") {
    preparedVideo = prepareSharedVideoPlayer(stage, file);
  ...
  try {
    ...
    if (sharedPreviewRequestActive(generation, file.id)) {
      ...
      else stage.innerHTML = `<p>${escapeHtml(error.message)}</p>`;
    }
  }
}

function nextSharedPlaybackMode(mediaKind) {
  if (mediaKind === "audio") {
...
function syncSharedPlaybackModeButton(button, media, mediaKind) {
  if (mediaKind !== "audio" && state.previewPlaybackMode === "continuous-audio") state.previewPlaybackMode = "off";
  const label = state.previewPlaybackMode === "repeat-one"
  ...
  button.classList.toggle("is-active", state.previewPlaybackMode !== "off");
}

function bindSharedPlaybackMode(button, media, file) {
  syncSharedPlaybackModeButton(button, media, file.mediaKind);
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    state.previewPlaybackMode = nextSharedPlaybackMode(file.mediaKind);
    syncSharedPlaybackModeButton(button, media, file.mediaKind);
  });
  if (file.mediaKind === "audio") {
    media.addEventListener("ended", async () => {
      if (state.previewPlaybackMode !== "continuous-audio") return;
      const audioFiles = state.files.filter((item) => item.mediaKind === "audio");
      const index = audioFiles.findIndex((item) => Number(item.id) === Number(file.id));
      const next = audioFiles[index + 1];
      if (!next) return;
      await openPreview(next, { pushHistory: false });
      await $("#preview-stage audio")?.play().catch(() => {});
    });
  }
}

function renderSharedAudioPlayer(stage, file, url) {
  const wrapper = document.createElement("div");
  wrapper.className = "preview-audio-player";
  const audio = document.createElement("audio");
  audio.controls = true;
  observeSharedMediaDuration(audio, file);
  const button = document.createElement("button");
  button.type = "button";
  button.className = "preview-playback-mode preview-audio-playback-mode";
  bindSharedPlaybackMode(button, audio, file);
  audio.src = url;
...
function sharedPreviewVideoIsFullscreen(sourceVideo = null) {
...
function recordOrientationLockFailure(mode, error, reason) {
  ...
  console.warn(`T-Cloud shared orientation lock failed (${mode}/${reason})`, error);
}
...
async function restoreInstalledAppPortrait({ settle = false, reason = "folder" } = {}) {
  ...
  for (const delay of retryDelays) {
    ...
    if (requestGeneration !== state.previewOrientationGeneration || state.previewVideoFullscreenActive || sharedPreviewVideoIsFullscreen()) return false;
    try {
...
async function prepareInstalledVideoFullscreen(sourceVideo = null) {
  const requestGeneration = ++state.previewOrientationGeneration;
  if (!isInstalledAppMode() || !screen.orientation?.lock || !sharedPreviewVideoIsFullscreen(sourceVideo)) return;
  state.previewVideoFullscreenActive = true;
  try {
    await screen.orientation.lock("any");
    if (requestGeneration !== state.previewOrientationGeneration || !state.previewVideoFullscreenActive || !sharedPreviewVideoIsFullscreen(sourceVideo)) {
      await restoreInstalledAppPortrait();
    }
  } catch (error) {
...
function enforceSharedFolderPortraitOrientation() {
  if (document.visibilityState === "hidden" || state.previewVideoFullscreenActive || sharedPreviewVideoIsFullscreen()) return;
...
function handleSharedPreviewFullscreenOrientationChange() {
  ...
  const fullscreenVideo = fullscreenElement?.matches?.("video")
    ? fullscreenElement
    : fullscreenElement?.querySelector?.("video");
  state.previewVideoFullscreenActive = Boolean(fullscreenVideo && sharedPreviewVideoIsFullscreen(fullscreenVideo));
...
function addSharedPreviewPlayerControls(stage, video, file) {
  ...
  const commitSeek = () => {
    ...
    video.currentTime = Math.max(0, Math.min(video.duration, pendingSeekSeconds));
  ...
  bindSharedPlaybackMode(modeButton, video, file);
...
function rememberSharedPreviewPointerType(event) {
  state.previewLastPointerType = event.pointerType || "mouse";
}

function toggleSharedPreviewVideoPlayback(video) {
  if (video.paused) video.play().catch(() => {});
  else video.pause();
}

function seekSharedPreviewVideoBy(video, seconds) {
...
function holdSharedPreviewControlsForDoubleTapSeek(stage) {
...
function handleSharedPreviewVideoTap(event) {
  const stage = $("#preview-stage");
  ...
  if (isDoubleTap) {
    ...
    if (action === "seek") {
      holdSharedPreviewControlsForDoubleTapSeek(stage);
      seekSharedPreviewVideoBy(video, side * DOUBLE_TAP_SEEK_SECONDS);
    }
    return;
  }
  const token = Symbol("share-preview-tap");
  const timer = setTimeout(() => {
    if (state.previewTapCandidate?.token !== token) return;
    state.previewTapCandidate = null;
    if (stage.querySelector("video") === video) toggleSharedPreviewVideoPlayback(video);
  }, 280);
  state.previewTapCandidate = { token, timer, at: now, x: event.clientX, y: event.clientY, side, pointerType };
}

function suppressSharedPreviewVideoDoubleClick(event) {
  if (!$("#preview-stage video")) return;
  event.preventDefault();
}

function clearSharedPreviewTapGesture() {
...
function prepareSharedVideoPlayer(stage, file) {
  ...
  addSharedPreviewPlayerControls(stage, video, file);
...
function showSharedVideoPlayerError(stage, buffering, text) {
...
function renderVideoPlayer(stage, file, url, generation) {
  loadSharedVideoPlayerSource(prepareSharedVideoPlayer(stage, file), file, url, generation);
}
