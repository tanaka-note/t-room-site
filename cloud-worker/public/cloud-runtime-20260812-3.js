...
const BACKGROUND_ITEM_PAGE_SIZE = 128;
...
async function loadItems() {
  ...
  try {
    if (state.view === "trash") {
      ...
      while (nextFolderOffset != null || nextFileOffset != null) {
        await nextRenderOpportunity();
        if (loadGeneration !== state.itemLoadGeneration) return { ok: false, stale: true, error: null };
        const folderPageParams = nextFolderOffset == null ? null : new URLSearchParams(params);
        const filePageParams = nextFileOffset == null ? null : new URLSearchParams(params);
        if (folderPageParams) {
          folderPageParams.set("pageSize", String(BACKGROUND_ITEM_PAGE_SIZE));
          folderPageParams.set("offset", String(nextFolderOffset));
          folderPageParams.set("foldersOnly", "1");
        }
        if (filePageParams) {
          filePageParams.set("pageSize", String(BACKGROUND_ITEM_PAGE_SIZE));
          filePageParams.set("offset", String(nextFileOffset));
          filePageParams.set("filesOnly", "1");
        }
        const [folderPage, filePage] = await Promise.all([
          folderPageParams ? api(`/items?${folderPageParams}`) : null,
          filePageParams ? api(`/items?${filePageParams}`) : null
        ]);
        if (loadGeneration !== state.itemLoadGeneration) return { ok: false, stale: true, error: null };
        const [pageFolders, pageFiles] = await Promise.all([
          hydrateFolderRecords(folderPage?.folders || [], { preserveOrder: true }),
          hydrateFileRecords(filePage?.files || [], { preserveOrder: true })
        ]);
        if (loadGeneration !== state.itemLoadGeneration) return { ok: false, stale: true, error: null };
        const foldersById = new Map(state.folders.map((folder) => [Number(folder.id), folder]));
        for (const folder of pageFolders) foldersById.set(Number(folder.id), folder);
        const filesById = new Map(state.files.map((file) => [Number(file.id), file]));
        for (const file of pageFiles) filesById.set(Number(file.id), file);
        state.folders = finalizeHydratedFolders([...foldersById.values()]);
        state.files = finalizeHydratedFiles([...filesById.values()]);
        renderItems();
        nextFolderOffset = normalizeNextItemOffset(folderPage?.nextFolderOffset);
        nextFileOffset = normalizeNextItemOffset(filePage?.nextFileOffset);
      }
      state.progressiveItemsLoading = false;
      loadVisibleEncryptedThumbnails();
...
function renderItems() {
  ...
  if (!state.progressiveItemsLoading) {
    scheduleMissingMediaDurations();
    scheduleMissingVideoThumbnails();
  }
...
function fileCard(file) {
  ...
  if (!state.progressiveItemsLoading && file.hasThumbnail && Number(file.cryptoVersion) === 1 && file.fileKey) {
    loadEncryptedThumbnail(file, card.querySelector(".thumb"));
  }
  return card;
}
...
function loadVisibleEncryptedThumbnails() {
  for (const file of state.files) {
    ...
    if (stage) loadEncryptedThumbnail(file, stage);
  }
}
