...
const APP_SHELL_CACHE = "tcloud-shell-20260811-5";
...
function constrainOpenEndedMp4Range(descriptor, rangeHeader, requested) {
  if (!requested?.partial || !/^bytes=\d+-$/.test(String(rangeHeader || "")) || !/^(video|audio)\/mp4$/i.test(String(descriptor.mimeType || ""))) {
    return requested;
  }
  const chunkSize = Number(descriptor.chunkSizeBytes || 8 * 1024 * 1024);
...
async function warmMediaForPlayback(token, entry) {
  ...
  for (let index = 1; index < maxChunks; index += 1) {
    ...
    await fetchAndDecryptChunk(entry, index, { prefetch: true }).catch(() => null);
    await Promise.resolve(entry.cacheWriteChain).catch(() => {});
    await wait(0);
  }
}
...
function shouldWarmTail(descriptor) {
  return /^(video\/(mp4|quicktime|webm)|audio\/mp4)$/i.test(String(descriptor.mimeType || ""));
}
