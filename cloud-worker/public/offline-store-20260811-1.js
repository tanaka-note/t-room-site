(function (global) {
  ...
  async function putChunk(id, index, bytes, options = {}) {
    ...
    if (!updated.offline && options.enforceCacheLimit !== false) await enforceCacheLimit(id, index);
  ...
  async function enforceCacheLimit(protectedId = "", protectedIndex = -1) {
    const db = await openDatabase();
    const entries = await requestResult(db.transaction(ENTRY_STORE).objectStore(ENTRY_STORE).getAll());
    let total = (entries || []).filter((entry) => !entry.offline).reduce((sum, entry) => sum + cachedBytes(entry), 0);
