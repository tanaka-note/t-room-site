...
assert.match(enterApp, /await prepareCryptoSession\(password, accountKey, passkeyContext\);[\s\S]*?\$\("#app-view"\)\.hidden = false;[\s\S]*?const loaded = await loadItems\(\);[\s\S]*?scheduleLegacyFolderMigration\(\);/);
