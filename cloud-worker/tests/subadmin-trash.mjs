...
assert.match(html, /id="logout-button-label">ログアウト<\/span>/);
assert.doesNotMatch(html, /id="mobile-logout-button"/);
assert.match(client, /const showLogoutLabel = state\.view === "all" && !insideFolder && !state\.kind;/);
assert.match(client, /\$\("#logout-button-label"\)\.hidden = !showLogoutLabel;/);
assert.match(client, /\$\("#logout-button"\)\.classList\.toggle\("icon-only", !showLogoutLabel\);/);
...
assert.match(client, /state\.canTrashCurrentFolderContents/);
assert.match(client, /Number\(file\?\.folderId\) === Number\(state\.folderId\)/);
assert.match(client, /!folder\.isProtected \|\| folder\.isUnlocked/);
assert.doesNotMatch(client, /function canTrashFile[\s\S]*?file\?\.fileKey[\s\S]*?function canTrashFolder/);
...
assert.match(worker, /const canTrashContents = Boolean\(folderId/);
assert.match(worker, /session\.canTrashUnlockedFiles && folderAccessGranted/);
