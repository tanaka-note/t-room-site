...
assert.match(client, /const FLOATING_TOOLBAR_DIRECTION_THRESHOLD = 12/);
assert.match(client, /if \(direction === "up"\) showFloatingToolbar\(\);\s*else hideFloatingToolbar\(\);/);
assert.match(client, /function floatingToolbarIsBeingUsed\(\)/);
assert.match(client, /toolbar\.contains\(document\.activeElement\) \|\| !\$\("#floating-location-panel"\)\.hidden/);
...
assert.match(client, /scheduleEncryptedThumbnailLoading\(\);\s*scheduleMissingMediaDurations\(\);\s*scheduleMissingVideoThumbnails\(\);\s*installItemRenderSentinel\(\);\s*queueFloatingToolbarUpdate\(\);/);
...
console.log("persistent compact folder toolbar below the page top: ok");
