...
await until(()=>entry.prefetchTask===null);
entry.prefetchedChunks.delete(23);stored.delete('A:1:23');
await context.test.fetchAndDecryptChunk(entry,20);
await until(()=>stored.has('A:1:23'));
assert.ok(requests.filter(index=>index===23).length>=2,'demand restarts prefetch after the previous task reached its window end');
