...
if (worker.includes("個別PWがない配下フォルダでは、作成後にPWを追加できません")) throw new Error("配下フォルダへの個別PW追加を妨げる旧制約が残っています。");
...
if (!client.includes('folder.isProtected && !folder.isUnlocked')
  || !client.includes('(!folder.isProtected || folder.isUnlocked) && folder.parentId')) {
  throw new Error("個別PW付き配下フォルダが親鍵だけで解除される可能性があります。");
}
...
if (!client.includes('folder.isProtected || folder.parentId')
  || !client.includes('"個別パスワードを設定"')
  || !html.includes('id="folder-password-replace-option"')) {
  throw new Error("既存の配下フォルダへ個別PWを追加する導線がありません。");
}
