...
for (const id of ["conflict-count-button", "floating-conflict-count-button", "conflict-dialog", "conflict-group-list", "conflict-file-list", "conflict-groups-back"]) {
  assert.match(html, new RegExp(`id="${id}"`), `${id} がありません。`);
}
assert.match(css, /\.file-card \.conflict-badge \{[^}]*top: 9px;[^}]*left: 9px;/);
assert.match(client, /if \(!state\.listMode && conflictGroupId && !file\.trashed\)/);
assert.match(client, /conflictBadge\.textContent = "競合"/);
...
assert.match(html, /cloud\.js\?v=20260810-96/);
