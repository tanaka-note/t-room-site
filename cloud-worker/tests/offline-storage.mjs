...
assert.match(worker, /DEMAND_PREFETCH_CHUNKS = 4/);
assert.match(worker, /PREFETCH_CONCURRENCY = 4/);
...
assert.doesNotMatch(worker, /BACKGROUND_PREFETCH_DELAY_MS/);
assert.match(worker, /entry\.prefetchReady = true/);
assert.match(worker, /MP4_METADATA_WARM_CHUNKS = 4/);
assert.match(worker, /MP4_RANGE_RESPONSE_LIMIT_BYTES = 2 \* 1024 \* 1024/);
assert.match(worker, /MP4_PLAYING_RANGE_LIMIT_BYTES = 64 \* 1024 \* 1024/);
assert.match(worker, /entry\.playing \? MP4_PLAYING_RANGE_LIMIT_BYTES : MP4_RANGE_RESPONSE_LIMIT_BYTES/);
assert.match(worker, /requested\.start \+ limit - 1/);
