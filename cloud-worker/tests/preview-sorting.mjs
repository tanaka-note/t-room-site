...
assert.match(mainHtml, /id="preview-rotate"[^>]*>↻ <span>自動回転<\/span>/);
assert.match(shareHtml, /id="share-preview-rotate"[^>]*>↻ <span>自動回転<\/span>/);
assert.match(mainHtml, /id="preview-stage-wrap"[\s\S]*?id="preview-rotate"/);
assert.match(shareHtml, /id="share-preview-stage-wrap"[\s\S]*?id="share-preview-rotate"/);
...
assert.match(mainJs, /previewAutoRotate/);
assert.match(shareJs, /previewAutoRotate/);
assert.match(mainJs, /function releasePreviewOrientationLock\(\)[\s\S]*?screen\.orientation\?\.unlock/);
assert.match(shareJs, /function releaseSharedOrientationLock\(\)[\s\S]*?screen\.orientation\?\.unlock/);
assert.doesNotMatch(mainJs.match(/async function togglePreviewRotation\(\)[\s\S]*?function releasePreviewOrientationLock/)?.[0] || "", /requestFullscreen/);
assert.doesNotMatch(shareJs.match(/async function toggleSharedPreviewRotation\(\)[\s\S]*?function releaseSharedOrientationLock/)?.[0] || "", /requestFullscreen/);
assert.doesNotMatch(mainJs, /screen\.orientation\.lock\("landscape"\)/);
assert.doesNotMatch(shareJs, /screen\.orientation\.lock\("landscape"\)/);
assert.match(mainJs, /resetPreviewRotation\(true\)/);
assert.match(shareJs, /resetSharedPreviewRotation\(true\)/);
assert.match(shareJs, /function preserveSharedPortraitOrientation\(force = false\)/);
...
assert.match(mainCss, /\.preview-rotate-overlay[\s\S]*?bottom: 54px/);
assert.match(shareCss, /\.preview-rotate-overlay[\s\S]*?bottom:54px/);
