...
assert.match(mainJs, /webkitbeginfullscreen[^\n]*handlePreviewVideoFullscreenEnter/);
assert.match(mainJs, /webkitendfullscreen[^\n]*handlePreviewVideoFullscreenExit/);
assert.match(shareJs, /webkitbeginfullscreen[^\n]*handleSharedVideoFullscreenEnter/);
assert.match(shareJs, /webkitendfullscreen[^\n]*handleSharedVideoFullscreenExit/);
assert.match(mainJs, /fullscreenVideo[\s\S]*?releasePreviewOrientationLock/);
assert.match(shareJs, /fullscreenVideo[\s\S]*?releaseSharedOrientationLock/);
assert.doesNotMatch(mainJs, /screen\.orientation\.lock\(/, "管理画面は端末の自動回転設定を上書きしないでください。");
assert.doesNotMatch(shareJs, /screen\.orientation\.lock\(/, "共有画面は端末の自動回転設定を上書きしないでください。");
assert.doesNotMatch(mainJs, /preserveAppOrientation/);
assert.doesNotMatch(shareJs, /preserveSharedPortraitOrientation/);
