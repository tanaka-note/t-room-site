...
assert.match(script, /await Promise\.all\(\[loadMeta\(\), loadEntries\(true\)\]\);\s*if \(returnView\) \{\s*await loadEntriesForDiaryReturn\(returnView\.entryCount\);\s*restoreDiaryReturnPosition\(returnView\.position\);/s);
