...
const [html, script, worker, manifest, serviceWorker, wrangler, icon192, icon512, maskable512, appleIcon] = await Promise.all([
  ...
  readFile(`${root}/public/icons/icon-192-v2.png`),
  readFile(`${root}/public/icons/icon-512-v2.png`),
  readFile(`${root}/public/icons/icon-maskable-512-v2.png`),
  readFile(`${root}/public/icons/apple-touch-icon-v2.png`)
]);
...
assert.ok(parsedManifest.icons.some((icon) => icon.purpose === "maskable" && icon.src.endsWith("icon-maskable-512-v2.png")));
...
assert.match(html, /name="troom-app-build" content="20260812-3"/);
assert.match(html, /apple-touch-icon-v2\.png/);
assert.match(html, /pwa-auto-update\.js\?v=20260812-3/);
...
assert.match(serviceWorker, /troom-diary-shell-v15/);
assert.match(serviceWorker, /icon-maskable-512-v2\.png/);
