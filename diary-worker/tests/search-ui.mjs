...
assert.match(html, /id="diary-date-from" type="text"[^>]*readonly/);
assert.match(html, /id="diary-date-to" type="text"[^>]*readonly/);
...
assert.match(script, /function handleDateClick\(event\) \{[\s\S]*event\.preventDefault\(\);[\s\S]*openDateWheel\(event\.currentTarget\);/);
assert.doesNotMatch(script, /useMobileDateWheel/);
assert.doesNotMatch(script, /openNativeDatePicker|showPicker|nativeDatePickerTarget/);
