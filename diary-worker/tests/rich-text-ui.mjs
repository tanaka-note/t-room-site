...
assert.match(beforeInputHandler, /document\.createElement\("br"\)/, "Enter時は画面上ですぐ次の行へ移る改行要素を挿入してください。");
assert.doesNotMatch(beforeInputHandler, /insertPlainTextAtEditorSelection\("\\n"\)/, "末尾で表示されない生の改行文字へ置き換えないでください。");
