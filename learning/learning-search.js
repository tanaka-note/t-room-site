(function () {
  const root = document.querySelector("[data-learning-search]");
  if (!root) return;
  ...
  async function runSearch() {
    ...
    updateUrl(query);
    if (!query) {
      setMessage("キーワードを入力すると、学習ログ記事の本文から探します。");
      return;
    }
    ...
    const learningItems = items.filter((item) => item.url && item.url.startsWith("/learning/sharoushi/logs/"));
    renderResults(learningItems);
  }
